...
\author{○塩田 大輔，鈴木 修馬，塩沢 悠星，小山 雄矢\\上田 隆一，林原 靖男（千葉工大）}
...
\eauthor{Daisuke SHIODA, Taisei YOSHINO, Shuma SUZUKI, Yusei SHIOZAWA, \\Yuya KOYAMA, Masato AITA, Ren SATO, Yuki TUBOUCHI, \\Ryuichi UEDA and Yasuo HAYASHIBARA (CIT)}
...
つくばチャレンジに参加している．
これまで本研究グループ\footnote{千葉工業大学未来ロボティクス学科 林原研究室，上田研究室}では，地図生成や自己位置推定のシステムを
中心に研究開発を行ってきた．近年では，防水機能や高い拡張性を有したオープンプラットフォームのロボットの
開発も行っている．

% つくばチャレンジ2024では, box2チームが本走行での完走を, box3チームが信号付近までの自律走行をそれぞれ目標として活動を行った\cite{sympo2024}.
昨年度のつくばチャレンジ2024では，box2チーム，box3チームともに本走行での完走を目標に取り組んでいた． 
また，box3チームは引き続き信号認識にも取り組んでいた\cite{sympo2024}．
しかし，両チームともに目標を達成することができなかった．
その要因としては，どちらも自己位置推定の破綻が挙げられている．
box2チームはランドマークの少ない公園内で，3次元点群で作成していた地図と違う地点で自己位置を合わせてしまったことが原因として挙げられた． 
box3チームはスキャンデータの信頼度に依存していたため，本走行とそれ以前の環境の差異に対応できなかったことが原因として挙げられた．
本稿では，昨2024年度の課題を解決するためにつくばチャレンジ2025に向けて取り組んだ内容を紹介する．
% 
% 諸言の中に読んだ人が何を得られるのかかく必要がある
\section{開発したロボット}
...
この問題を解決するべく，本研究グループでは屋外自律移動ロボットプラットフォーム ORNE-boxの開発を行ってきた\cite{box}．
...
\subsection{ソフトウェア}
本研究グループでは，従来より ROS(Robot Operating System)\cite{ros}のnavigation stack\cite{navigation}を基に開発された
...
本研究グループでは，使用するロボットごとに研究開発の目的が異なる．つくばチャレンジ2025においては，
% ORNE-box2を使用するチームとORNE-box3を使用するチームの2つに分かれて研究開発を行ってきた．
ORNE-box，ORNE-box2，ORNE-box3，ORNE-gamma，F号ボボッボ甲，それぞれのロボットを使用する5つのチームに分かれて研究開発を行ってきた．
以降，ORNE-box2，ORNE-box3を使用するチームをbox2チーム，box3チームと呼ぶこととする．
本章では，box2 チーム，box3チームチームごとの取り組みを述べる．
...
本2025年度のbox2チームではROS Navigation stackを中心としたソフトウェアを用いて, 
...
自律走行時に使用する地図の作成には, Slam\_Toolbox\cite{Slam_Toolbox}と画像編集ソフトのgimp\cite{gimp}を用いた. 
...
最適化することができる．\par
なお，パラメータの調整方法は本研究室のGitHubリポジトリで公開している(\url{https://github.com/open-rdc/nav_tuning_guide})
...
具体的な対策としては, カメラを用いた動的障害物の検出などが挙げられる. 
なお, box2チームは確認走行を突破している. 
...
リタイア時の様子を\figref{fig:box3-failed}に示す.
...
具体的には市庁舎裏の狭い通路に進入する際に目標経路を通過してしまい, 
自己位置の修正を試みる挙動として, 
ロボットがその場で1回転する動作が生じた．
